...
function App() {
  ...
  const getRandomDogInfo = () => {
    setIsLoading(true);
    fetch("https://dog.ceo/api/breeds/random/info")
      .then((response) => {
        if (response.status === 200 || response.ok) {
          return response.json();
        } else {
      ...
      <main role="main">
        ...
        {randomDogInfo !== "" && dogImages.length === 0 && !isLoading ? (
          <div className="mt-5">
            ...
            <p>Name: {randomDogInfo.name}</p>
            <p>Age: {randomDogInfo.age}</p>
            <p>Temperament: {randomDogInfo.temperament}</p>
          </div>
        ) : null}
